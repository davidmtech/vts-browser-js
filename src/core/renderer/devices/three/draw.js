...
const ThreeDraw = function(renderer) {
...
ThreeDraw.prototype.drawSkydome = function(texture, shader) {
    if (!texture) {
        return;
    }
...
ThreeDraw.prototype.drawTBall = function(position, size, shader, texture, size2, nocull) {
    const gpu = this.gpu;
    //const gl = this.gl;
    const renderer = this.renderer;
...
ThreeDraw.prototype.drawBall = function(position, size, size2, shader, params, params2, params3, color, color2, normals) {
    const gpu = this.gpu;
    const gl = this.gl;
    const renderer = this.renderer;
...
ThreeDraw.prototype.drawBall2 = function(position, size, shader, nfactor, dir, radius2) {
    const gpu = this.gpu;
    const renderer = this.renderer;
...
ThreeDraw.prototype.drawLineString = function(points, screenSpace, size, color, depthOffset, depthTest, transparent, writeDepth, useState) {
    const gpu = this.gpu;
    //const gl = this.gl;
    const renderer = this.renderer;
    let index = 0, p, i;
    ...
    const plineBuffer = renderer.plineBuffer;
    ...
    const prog = (!screenSpace) ? renderer.gpu.progLine5 : renderer.gpu.progLine4;
...
ThreeDraw.prototype.drawCircle = function(point, radius, lineWidth, color, depthOffset, depthTest, transparent, writeDepth, useState) {
    const points = [];
...
ThreeDraw.prototype.drawImage = function(x, y, lx, ly, texture, color, depth, depthOffset, depthTest, transparent, writeDepth, useState) {
    const gpu = this.gpu;
    const gl = this.gl;
    const renderer = this.renderer;
    ...
    const prog = renderer.gpu.progImage;

    gpu.useProgram(prog, ['aPosition']);
    gpu.bindTexture(texture);

    const vertices = renderer.rectVerticesBuffer;
    gl.bindBuffer(gl.ARRAY_BUFFER, vertices);
    gl.vertexAttribPointer(prog.getAttribute('aPosition'), vertices.itemSize, gl.FLOAT, false, 0, 0);

    const indices = renderer.rectIndicesBuffer;
...
ThreeDraw.prototype.drawBillboard = function(mvp, texture, color, depthOffset, depthTest, transparent, writeDepth, useState) {
    const gpu = this.gpu;
    const gl = this.gl;
    const renderer = this.renderer;
    let tmpState;
...
ThreeDraw.prototype.drawFlatImage = function(x, y, lx, ly, texture, color, depth, depthOffset) {
    const gpu = this.gpu;
    const gl = this.gl;
    const renderer = this.renderer;
...
ThreeDraw.prototype.drawText = function(x, y, size, text, color, depth, useState) {
    const gpu = this.gpu;
    const gl = this.gl;
    const renderer = this.renderer;
    let tmpState;
...
ThreeDraw.prototype.getTextSize = function(size, text) {
...
ThreeDraw.prototype.drawGpuJobs = function() {
    const gpu = this.gpu;
    const gl = this.gl;
    const renderer = this.renderer;
...
ThreeDraw.prototype.clearJobBuffer = function() {
    const renderer = this.renderer;
    const jobZBuffer = renderer.jobZBuffer;
    const jobZBufferSize = renderer.jobZBufferSize;
...
ThreeDraw.prototype.clearJobHBuffer = function() {
    const renderer = this.renderer;
    const jobZBuffer2 = renderer.jobZBuffer2;
    const jobZBuffer2Size = renderer.jobZBuffer2Size;
...
ThreeDraw.prototype.paintGL = function() {   //remove this??
    const renderer = this.renderer;
...
ThreeDraw.prototype.processNoOverlap = function(renderer, job, pp, p1, p2, camVec, l, stickShift, texture, files, color) {
    const res = {
        exit: true
    };
...
ThreeDraw.prototype.drawGpuJob = function(gpu, gl, renderer, job, screenPixelSize, advancedHitPass, ignoreFilters) {
    if (!job.ready) {
        return;
    }
    ...
    case VTS_JOB_POLYGON:
        {
            ...
            if (useSuperElevation) {
                prog = advancedHitPass ? job.program2 : renderer.gpu.progLineSE;
            } else {
                prog = advancedHitPass ? job.program2 : debugWires ? renderer.gpu.progLineWireframe : job.program;
            }

            const flatShade = (!advancedHitPass && job.type == VTS_JOB_POLYGON && job.style == 1);

            if (flatShade) {
                prog = useSuperElevation ? renderer.gpu.progCFlatShadeTileSE : renderer.gpu.progCFlatShadeTile;
            }
            ...
            if (drawDebugLines) {
                ...
                const program = useSuperElevation ? renderer.gpu.progWireFrameBasicSE : renderer.gpu.progWireFrameBasic;
    ...
    case VTS_JOB_PIXEL_TLINE:
        {
            ...
            if (hitmapRender) {
                if (job.type == VTS_JOB_PIXEL_TLINE) {
                    ...
                    prog = advancedHitPass ? this.renderer.gpu.progELine3 : this.renderer.gpu.progLine3;
                    if (!prog.isReady()) {
                        return;
                    }
                }
            ...
            if (useSuperElevation) {
                prog = advancedHitPass ? job.program2 : renderer.gpu.progLine3SE;
    ...
    case VTS_JOB_LINE_LABEL:
        {
            ...
            if (job.singleBuffer) {
                ...
                /*
                if (bl > 384) { vbuff = renderer.textQuads128; prog = renderer.gpu.progLineLabel128; } else
                if (bl > 256) { vbuff = renderer.textQuads96; prog = renderer.gpu.progLineLabel96; } else
                if (bl > 192) { vbuff = renderer.textQuads64; prog = renderer.gpu.progLineLabel64; } else
                if (bl > 128) { vbuff = renderer.textQuads48; prog = renderer.gpu.progLineLabel48; } else
                if (bl > 64) { vbuff = renderer.textQuads32; prog = renderer.gpu.progLineLabel32; }
                else { vbuff = renderer.textQuads16; prog = renderer.gpu.progLineLabel16; }
            ...
            prog = renderer.useSuperElevation ? renderer.gpu.progText2SE : job.program;
            gpu.useProgram(prog, ['aPosition', 'aTexCoord']);
    ...
    case VTS_JOB_VSPOINT:
        {
            ...
            if (job.singleBuffer) {
                ...
                if (prog == renderer.gpu.progIcon) {
                    const b = job.singleBuffer;
                    prog = renderer.gpu.progImage;
                    ...
                    if (bl > 384) { vbuff = renderer.textQuads128; prog = renderer.gpu.progLabel128; } else
                    if (bl > 256) { vbuff = renderer.textQuads96; prog = renderer.gpu.progLabel96; } else
                    if (bl > 192) { vbuff = renderer.textQuads64; prog = renderer.gpu.progLabel64; } else
                    if (bl > 128) { vbuff = renderer.textQuads48; prog = renderer.gpu.progLabel48; } else
                    if (bl > 64) { vbuff = renderer.textQuads32; prog = renderer.gpu.progLabel32; }
                    else { vbuff = renderer.textQuads16; prog = renderer.gpu.progLabel16; }
...
ThreeDraw.prototype.drawGpuSubJob = function(gpu, gl, renderer, screenPixelSize, subjob, fade) {
    if (!subjob) {
        return;
    }
    ...
    let prog = job.program; //renderer.gpu.progIcon;
...
ThreeDraw.prototype.drawGpuSubJobLineLabel = function(gpu, gl, renderer, screenPixelSize, subjob, fade) {
    if (!subjob) {
        return;
    }
